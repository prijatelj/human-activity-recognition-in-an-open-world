...
def make_dataset(split_file, split, root, predict="class"):
    ...
    video_names = glob.glob(root + "/*.mp4")
    ...
    return dataset,len(idx_to_class),idx_to_class
...
class PAR(data_utl.Dataset):
    ...
    # def __init__(self, split_file, split, root, num_classes=101, spatial_transform=None, task='class', frames=80, gamma_tau=5, crops=1):
        ...
        self.data,self.no_classes,self.idx_to_class = make_dataset(split_file, split, root, task)
...
def custom_collate_fn(batch):
    ...
    for b in batch:
        ...
        new_batch.append([torch.from_numpy(clips), label])
