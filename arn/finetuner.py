...
def finetune(features, labels, model, epochs=10, batch_size=5,verbose=False, save_path=""):
    ...
    for epoch in range(epochs):
        ...
        print("Train Accuracy: " + str(right/t_len))
        right = 0
        tot_cls_loss = 0.0
...
data = pd.read_csv('/home/sgrieggs/PycharmProjects/arn/arn/par_temp_train.csv', index_col=False,header=0)
...
for i,row in data.iterrows():
    target = "/media/sgrieggs/scratch365/humongous_big_dumps/par_kinetics/" + row['anonymous_id'].split(".")[0]+"_logits.pt"
